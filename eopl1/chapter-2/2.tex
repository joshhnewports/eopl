...
2.3.2 (extends 2.3.1)
We stipulate that a variable having no references in an expression is not free. The same follows for a variable not being bound.
\begin{lstlisting}
(define free?
  (lambda (var exp)
    (if (varref? exp)
...
$exp[y/x] \equiv exp[x \mapsfrom y]$, {\tt (rename exp x y)}
\begin{lstlisting}
(define rename 
